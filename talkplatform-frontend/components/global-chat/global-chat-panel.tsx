...
export function GlobalChatPanel({ className }: GlobalChatPanelProps) {
  ...
  return (
    <div className={`flex flex-col h-full min-h-0 ${className || ''}`}>
      ...
      {!isConnected && (
        <div className="bg-yellow-900/30 px-3 py-2 text-xs text-yellow-300 text-center flex-shrink-0 border-b border-yellow-800">
        ...
        <ScrollArea className="h-full p-3">
          <div className="space-y-1">
            {messages.map((message, index) => {
              ...
              const isOwnMessage = user?.id === message.sender.user_id;
              ...
              return (
                <div key={message.id} className={`w-full ${isOwnMessage ? 'flex justify-end' : ''}`}>
                  <div className={`max-w-[80%] ${isOwnMessage ? 'text-right' : ''}`}>
                    ...
                    {showHeader && (
                      <div className={`flex items-baseline gap-2 mb-1 mt-3 ${isOwnMessage ? 'justify-end' : ''}`}>
                    ...
                    {/* Message content */}
                    <div className={`text-sm text-gray-200 mb-1 break-words px-3 py-2 rounded-lg ${
                      isOwnMessage 
