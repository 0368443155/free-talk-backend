...
export const YouTubePlayer = forwardRef<YouTubePlayerHandle, YouTubePlayerProps>(function YouTubePlayer(
  ...
  useEffect(() => {
    ...
    // Only skip if the exact same video is already loaded and playing/paused correctly
    const currentVideoId =
      typeof player.getVideoData === "function" ? player.getVideoData()?.video_id : null;

    // Changed condition: only skip if video is the same AND last loaded is also the same
    // This ensures video loads even after clear
    if (lastLoadedVideoId.current === videoId && currentVideoId === videoId) {
      // But still handle play/pause state changes
    ...
    console.log("🎬 Loading new video:", videoId, "lastLoaded:", lastLoadedVideoId.current);
  ...
  const loadAndBroadcastVideo = (
    ...
    // Force reset lastLoadedVideoId to ensure video loads
    lastLoadedVideoId.current = null;
  ...
  useEffect(() => {
    ...
    const handleYouTubeSync = (data: {
      ...
      // Participants should always sync to host's state
      // Host should only ignore if this is their own recent action

      isLocalChange.current = true;

      if (data.videoId && data.videoId !== videoId) {
        console.log("🔄 Syncing to new video:", data.videoId);
        setVideoId(data.videoId);
        lastLoadedVideoId.current = null; // Reset to force load
    ...
    const handleYouTubePlay = (data: { videoId?: string; currentTime: number; userId: string }) => {
      ...
      // Don't process if this is the host's own action (isLocalChange handles this)
      // But DO process if it's a video change from host to sync everyone
      
      isLocalChange.current = true;

      if (data.videoId && data.videoId !== videoId) {
        console.log("🔄 Loading new video from play event:", data.videoId);
        setVideoId(data.videoId);
        lastLoadedVideoId.current = null; // Reset to force load
