...
export default function CreateCoursePage() {
    ...
    const addMaterial = (sessionId: string, type: MaterialType) => {
        ...
        const newMaterial: MaterialFile = {
            ...
            description: '',
            is_required: false,
            display_order: session.materials.length,
        };
    ...
    const removeMaterial = (sessionId: string, materialId: string) => {
        const session = sessions.find(s => s.id === sessionId);
        if (!session) return;

        updateSession(sessionId, {
            materials: session.materials.filter(m => m.id !== materialId),
        });
    };

    const updateMaterial = (sessionId: string, materialId: string, updates: Partial<MaterialFile>) => {
        const session = sessions.find(s => s.id === sessionId);
        if (!session) return;
    ...
    const handleFileUpload = async (sessionId: string, materialId: string, file: File) => {
        setUploadingMaterials(prev => ({ ...prev, [`${sessionId}-${materialId}`]: true }));
        ...
        try {
            ...
            updateMaterial(sessionId, materialId, {
                file,
                file_url: url,
                file_name: file.name,
                file_size: size,
            ...
            setUploadingMaterials(prev => ({ ...prev, [`${sessionId}-${materialId}`]: false }));
        }
    };
    ...
    const handleSubmit = async () => {
        ...
        for (const session of sessions) {
            if (!session.title || !session.scheduled_date || !session.start_time || !session.end_time) {
                toast({
                    title: 'Validation Error',
                    description: `Session ${session.session_number} is missing required fields`,
                    variant: 'destructive',
                });
                return;
            }
            ...
            for (const session of sessions) {
                for (const material of session.materials) {
                    if (material.file && !material.uploaded) {
                        await handleFileUpload(session.id, material.id, material.file);
                    }
                }
            }
                    ...
                    <CardHeader>
                        <div className="flex justify-between items-center">
                            <div>
                                ...
                                <CardDescription>
                                    Add sessions with materials for your course
                                </CardDescription>
                            </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(e) => updateSession(session.id, { scheduled_date: e.target.value })}
                                                    className="mt-1"
                                                />
                                            </div>
                                        </div>
                                            ...
                                            <div className="flex justify-between items-center mb-3">
                                                ...
                                                <div className="flex gap-2">
                                                    <Button
                                                        ...
                                                        variant="outline"
                                                        size="sm"
                                                        onClick={() => addMaterial(session.id, MaterialType.DOCUMENT)}
                                                    >
                                                    ...
                                                    <Button
                                                        ...
                                                        variant="outline"
                                                        size="sm"
                                                        onClick={() => addMaterial(session.id, MaterialType.VIDEO)}
                                                    >
                                                    ...
                                                    <Button
                                                        ...
                                                        variant="outline"
                                                        size="sm"
                                                        onClick={() => addMaterial(session.id, MaterialType.LINK)}
                                                    >
                                                        ...
                                                        <Card key={material.id} className="p-3">
                                                            ...
                                                            <div className="space-y-2">
                                                                <Input
                                                                    ...
                                                                    onChange={(e) => updateMaterial(session.id, material.id, { title: e.target.value })}
                                                                />
                                                                <Textarea
                                                                    placeholder="Description (optional)"
                                                                    value={material.description || ''}
                                                                    onChange={(e) => updateMaterial(session.id, material.id, { description: e.target.value })}
                                                                ...
                                                                {material.type !== MaterialType.LINK && (
                                                                    <div>
                                                                        ...
                                                                        <div className="border-2 border-dashed rounded-lg p-4 text-center hover:bg-gray-50 transition-colors cursor-pointer relative mt-1">
                                                                            <input
                                                                                type="file"
                                                                                ...
                                                                                onChange={(e) => {
                                                                                    ...
                                                                                    if (file) {
                                                                                        updateMaterial(session.id, material.id, { file_name: file.name });
                                                                                        handleFileUpload(session.id, material.id, file);
                                                                                    }
                                                                                }}
                                                                                disabled={uploadingMaterials[`${session.id}-${material.id}`]}
                                                                            />
                                                                            <div className="flex flex-col items-center gap-2">
                                                                                {uploadingMaterials[`${session.id}-${material.id}`] ? (
                                                                                    <>
                                                                                    ...
                                                                                    <>
                                                                                        ...
                                                                                        <span className="text-sm font-medium text-green-600">{material.file_name}</span>
                                                                                    </>
                                                                                ) : (
                                                                                    <>
                                                                ...
                                                                {material.type === MaterialType.LINK && (
                                                                    <Input
                                                                        ...
                                                                        onChange={(e) => updateMaterial(session.id, material.id, { file_url: e.target.value })}
                                                                    />
                                                                )}
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    <input
                                                                        ...
                                                                        onChange={(e) => updateMaterial(session.id, material.id, { is_required: e.target.checked })}
                                                                        className="w-4 h-4"
                                                                    />
                                                                    <Label htmlFor={`required-${material.id}`} className="text-sm font-normal cursor-pointer">
                                                                        Required material
                                                                    </Label>
                                                                </div>
                                                            </div>
