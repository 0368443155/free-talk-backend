...
import { loginApi } from "@/api/user.rest";
import { useUser } from "@/store/user-store";
import { useToast } from "@/components/ui/use-toast";
import { setCookie } from 'cookies-next';
// SỬA LỖI: Xóa setCookie vì chúng ta thống nhất dùng localStorage
// import { setCookie } from "cookies-next";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      const errorMessages: Record<string, string> = {
        ...
        'no_email': 'Unable to get email from Google account.',
        'backend_auth_failed': 'Failed to authenticate with server. Please try again.',
        'oauth_failed': 'Google authentication failed. Please try again.',
        'oauth_init_failed': 'Failed to start Google authentication.',
        'oauth_config_missing': 'Google authentication is not configured properly.',
        'oauth_callback_failed': 'Failed to complete Google authentication.',
      };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('📝 Setting user info to store...');
      ...
      // Hard redirect to dashboard to ensure navigation
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                required
                disabled={isLoading}
                className="h-11 bg-white dark:bg-gray-900 border-gray-300 dark:border-gray-700 focus:border-blue-500 focus:ring-blue-500"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  required
                  disabled={isLoading}
                  className="h-11 bg-white dark:bg-gray-900 border-gray-300 dark:border-gray-700 focus:border-blue-500 focus:ring-blue-500 pr-10"
                />
                <Button
                  type="button"
                  variant="ghost"
